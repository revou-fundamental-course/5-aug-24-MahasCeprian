const html2canvasMin = require("./html2canvas.min");
...
function showSuggestions(category) {
    ...
    if (category === 'underweight') {
        suggestionsElement.innerHTML += '<div class="vn-blue" style="text-align: center;"><a href="#" class="btn-wrap" id="image-download;">DOWNLOAD</a></div>';
...
// Fungsi tombol download image
document.getElementById("image-download").onclick = function(){
    const screenshootTarget = document.getElementById("print-image");

    html2canvas(screenshootTarget).then((canvas) => {
        const base64image = canvas.toDataURL("image/png");
        var anchor  = document.createElement('a');
        anchor.setAttribute("href", base64image);
        anchor.setAttribute("download", "Hasil_BMI_Calculator");
        anchor.click();
        anchor.remove();
    });
};